...
#define CLIENT_TIMEOUT   5   /*客户端超时断开时间 单位 S*/
...
/*客户端超时处理函数*/
void timer_func(client_data* user_data)
{
	if(user_data!=NULL)
	{
		printf("client %d close becase of timeout",user_data->sockfd);
		close(user_data->sockfd);
	//	wh_timer.del_timer(user_data->timer);
	  ...
	  for(int i=0;i<number;i++)
	  {
		 ...
		 if(socketfd==listenfd)
		 {
			 ...
			 if(connfd>=0)
			 {
				 ...
				 user_data[connfd].timer=wh_timer.add_timer(CLIENT_TIMEOUT,&user_data[connfd]);
